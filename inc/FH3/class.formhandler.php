...
class FormHandler
{
      ...
      $config    = null)
    {
        ...
        require_once(FH_INCLUDE_DIR.'includes/fckeditor.php');
      ...
      $extra     = null)
    {
        ...
        // create a new datefield
        $fld =& new jsDateField( $this, $name, $mask, $required, $interval );
    ...
    function cancelButton($caption = null, $url = null, $name = null, $extra = null)
    {
        ...
        $extra .= ereg('history', $url) ? ' onclick="'.$url.'"' : ' onclick="document.location.href=\''.$url.'\'"';
    ...
    function setAutoComplete( $field, $options )
    {
        ...
        $this->_fields[$field][1]->_sExtra .= " onkeypress='return autocomplete(this, event, ".$field."_values);' ";
    }
    ...
    function setTabIndex( $tabs )
    {
        ...
        else if( is_array( $tabs ))
        {
            ...
            if( isset( $tabs[0]))
            {
                ...
                {
                    while( array_key_exists( $key, $tabs) || $key <= 0) $key++;
                    $tabs[$key] = $value;
                }
            }
    ...
    */
    ...
    {
        // does the field exists?
        if( $this -> fieldExists( $field ) && method_exists( $this -> _fields[$field][1], 'getViewMode' ) )
        {
            // get the mode
    ...
    */
    ...
    function setValue( $sField, $sValue, $bOverwriteCurrentValue = false )
    {
        ...
        if( $this->fieldExists( $sField ) )
        {
            ...
            if( $bOverwriteCurrentValue || !$this->isPosted() ) //|| $this->_curPage < $this->_pageCounter ) # why was this ?
            {
     ...
     * FormHandler::getPage()
     *
     * Returns the page number of the current form (used when newPage is used!)
     *
     * @return int
     * @access public
     * @author Teye Heimans
        ...
        function linkSelectFields( $filename, $fields )
    {
        ...
        if( $this -> _curPage != ($this -> _pageCounter-1 < 1 ? 1 : $this -> _pageCounter-1) ||
        ($this -> isPosted() && !$this -> isCorrect() ) )
        {
            return ;
        }
    ...
    function isCorrect()
    {
        ...
        foreach( $this->_fields as $id => $data )
        {
            ...
            if( method_exists( $this->_fields[$id][1], 'isvalid') && $this->_fields[$id][1]->isValid() !== true)
            {
    ...
    function _getForm( $iDisplayPage = null )
    {
            ...
            switch( $field[0] )
            {
                ...
                else
                {
                   ...
                   */
                    ...
                    // can we get a field ?
                    if( method_exists($obj, 'getField') )
                    {
                        $fld = $obj -> getField();
                    }
                    // can we get a button ?
                    else if( method_exists($obj, 'getButton') )
                    {
                        $fld = $obj -> getButton();
                    }
                   ...
                   */
                    ...
                    $error = '';
                    if( $this->_displayErrors && method_exists($obj, 'getError') )
                    {
                        ...
                        if( array_key_exists($name, $this->_customMsg) && $obj->getError() != '' )
                        {
                            ...
                            {
                                $error = sprintf( FH_ERROR_MASK, $this->_customMsg[$name][0] );
                            }
        ...
        ( $this->_setTable ?
        sprintf(
        "<table border='%d' cellspacing='%d' cellpadding='%d' align='center' %s>\n",
        ...
        ( $this->_setTable ? "\n</table>\n" : '').
        (FH_EXPOSE ?
        "<small><font color='B5B5B5' face='tahoma' style='font-family:tahoma;font-size:10px;color:B5B5B5;font-weight:normal;'>".
        "This form is generated by </font></small><a href='http://www.formhandler.net' target='_blank'>".
        "<font color='B5B5B5' face='tahoma' style='font-family:Tahoma;font-size:10px;color:B5B5B5;font-weight:normal;'><b><small>FormHandler</small></b></font></a>" :''
        ).
        "</form>\n".
        "<!--\n".
        "  This form is automaticly being generated by FormHandler v3.\n".
        ...
        $result .= $this -> getJavascriptCode( false );
